  ...
  return (
    <div className="py-12 px-4 md:px-8 max-w-7xl mx-auto space-y-10">
      ...
      <div className="text-center max-w-2xl mx-auto space-y-4">
        <span className="text-[#616F3E] font-mono uppercase tracking-widest text-[10px] font-semibold border border-[#E4DFD3] px-3 py-1 rounded-full bg-[#FAF8F5]">
          Smart Stacks
        </span>
        <h1 className="text-4xl md:text-5xl font-serif text-[#203348]">Supplement Combos</h1>
      ...
      <div className="flex justify-center mb-8">
        <div className="inline-flex bg-[#F2EEE5] border border-[#E4DFD3] p-1 rounded-full">
          <button
          ...
          <button
            ...
            className={`px-6 py-2.5 rounded-full text-xs font-semibold uppercase tracking-wider transition-colors cursor-pointer ${
              activeTab === 'build-your-own' ? 'bg-[#203348] text-white shadow-sm' : 'text-[#203348]/60 hover:text-[#203348]'
            }`}
          >
            Make Your Own Stack
          </button>
      ...
      {activeTab === 'build-your-own' && (
        <div className="max-w-3xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className="relative overflow-hidden bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-[#E4DFD3] text-center">
            <div 
              className="absolute inset-0 pointer-events-none opacity-[0.04] bg-repeat"
              style={{ backgroundImage: "url('/patterns/pattern-rust.jpg')", backgroundSize: '280px auto' }}
            />
            <div className="relative z-10">
              <h2 className="font-serif text-2xl text-[#203348] mb-3">Tell us about your routine</h2>
              <p className="text-sm text-[#203348]/70 mb-6">Describe your health goals, current diet, or the issues you want to address. We'll suggest a custom stack and give you a 15% discount when you buy them together.</p>
            </div>
            ...
            <textarea
              ...
              className="w-full h-32 p-4 bg-[#FAF8F5] border border-[#E4DFD3] rounded-xl text-sm focus:outline-none focus:border-[#616F3E] mb-4 resize-none placeholder-[#203348]/40 text-[#203348]"
            ></textarea>
            
            <button
              onClick={handleGenerateCustomStack}
              disabled={isAnalyzing || !customInput.trim()}
              className="bg-[#A5492B] text-white hover:bg-[#203348] px-8 py-3.5 rounded-full text-xs font-semibold uppercase tracking-widest transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed w-full md:w-auto"
            >
          ...
          {customStack && (
            <div className="animate-in fade-in zoom-in-95 duration-500">
              <h3 className="font-serif text-xl text-center text-[#203348] mb-6">Your Recommended Plan</h3>
              <div 
                className="group relative flex flex-col md:flex-row bg-white rounded-2xl transition-all shadow-md border border-[#616F3E]/20 overflow-hidden"
              >
                {/* Visual stacked presentation of bottles */}
                <div className="relative w-full md:w-1/3 h-64 md:h-auto overflow-hidden bg-[#F2EEE5] flex items-center justify-center border-b md:border-b-0 md:border-r border-[#E4DFD3]">
                    ...
                    {customStack.productIds.map((id, index) => {
                      ...
                      return (
                        <React.Fragment key={prod.id}>
                          {index > 0 && (
                            <div className="text-2xl font-black text-[#616F3E]/60 mx-2 md:mx-3">+</div>
                          )}
                          <div 
                  ...
                  <div className="absolute top-4 right-4 bg-[#A5492B] text-white text-[10px] font-mono px-3 py-1 rounded-full shadow-md font-bold tracking-widest uppercase z-40">
                    Save 15%
                  </div>
                </div>
                ...
                <div className="flex-grow flex flex-col justify-between p-6">
                  ...
                  <div className="mt-6 pt-4 border-t border-[#E4DFD3] flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                      <span className="text-[10px] text-[#203348]/50 uppercase block font-mono">Custom Combo Price</span>
                      <div className="flex items-baseline space-x-2 mt-0.5">
                        <span className="font-mono text-2xl font-bold text-[#203348]">₹{customStack.comboPrice}</span>
                        <span className="font-mono text-sm text-[#203348]/40 line-through">₹{customStack.originalPrice}</span>
                      </div>
                    </div>
                    <button
                      onClick={() => handleAddBundleToCart(customStack)}
                      className="px-6 py-3 rounded-full text-xs font-semibold uppercase tracking-wider transition-all duration-300 bg-[#A5492B] text-white hover:bg-[#203348] shadow-md cursor-pointer"
                    >
                      Add Custom Stack
                    </button>
                  </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
          {stacks.map((bundle) => {
            ...
            return (
              <div 
                key={bundle.id}
                className="group relative flex flex-col h-full rounded-2xl bg-white transition-all duration-300 hover:-translate-y-1.5 hover:shadow-lg border border-[#E4DFD3] overflow-hidden"
              >
                {/* Product Bottle Image Area */}
                <div className="relative w-full h-64 overflow-hidden border-b border-[#E4DFD3] bg-[#F2EEE5] flex items-center justify-center group-hover:bg-[#E4DFD3]/20 transition-colors">
                  
                  {/* Dynamic Background */}
                  <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(255,255,255,0.8)_0%,transparent_100%)] opacity-80"></div>
                  <div className="absolute -top-24 -right-24 w-64 h-64 bg-[#616F3E]/20 rounded-full blur-3xl"></div>
                  <div className="absolute -bottom-24 -left-24 w-64 h-64 bg-[#203348]/10 rounded-full blur-3xl"></div>
                ...
                <div className="flex-grow flex flex-col justify-between p-5 pt-4 text-left">
                  ...
                  <div className="mt-4 pt-3 border-t border-[#E4DFD3] flex items-center justify-between">
                    <div>
                      <span className="text-[9px] text-[#203348]/40 uppercase block leading-none font-mono">Combo Price</span>
                      <div className="flex items-baseline space-x-1.5 mt-0.5">
                        <span className="font-mono text-base font-bold text-[#203348]">₹{currentPrice}</span>
                        <span className="font-mono text-[10px] text-[#203348]/40 line-through">₹{originalPrice}</span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center space-x-2">
                      <button
                        onClick={() => setSelectedDetailStack(bundle)}
                        className="text-[11px] font-semibold text-[#203348]/60 hover:text-[#203348] transition-colors uppercase tracking-wider py-1 cursor-pointer"
                      >
                        Info
                      </button>
                      ...
                      <button
                        ...
                        className="px-4 py-2 rounded-full text-xs font-semibold uppercase tracking-wider transition-all duration-300 bg-[#A5492B] text-white hover:bg-[#203348] shadow-sm cursor-pointer"
                      >
                        Buy Combo
                      </button>
                    </div>
      ...
      {selectedDetailStack && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-[#203348]/60 backdrop-blur-sm transition-opacity duration-300">
          <div className="bg-[#FAF8F5] max-w-2xl w-full rounded-3xl overflow-hidden border border-[#E4DFD3] shadow-2xl flex flex-col animate-in fade-in zoom-in-95 duration-200 max-h-[90vh]">
            
            {/* Modal Header */}
            <div className="p-6 bg-white border-b border-[#E4DFD3] flex justify-between items-center text-left">
              <div>
                <span className="text-[9px] font-mono font-bold uppercase tracking-wider text-[#616F3E] bg-[#616F3E]/10 px-2 py-0.5 rounded">
                  Combo Details
                </span>
                <h3 className="font-serif text-2xl font-bold text-[#203348] mt-1">
                  {selectedDetailStack.name}
                </h3>
              </div>
              <button 
                onClick={() => setSelectedDetailStack(null)}
                className="text-[#203348]/40 hover:text-[#203348] text-xl p-1 cursor-pointer transition-colors"
              >
                ✕
              </button>
            </div>
            ...
            <div className="p-6 overflow-y-auto space-y-6 text-left">
              ...
              <div className="space-y-3">
                <span className="text-[9px] font-mono font-bold uppercase text-[#203348]/40 block">Products in this Combo</span>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  {selectedDetailStack.productIds.map(id => products.find(p => p.id === id)).filter(Boolean).map((prod) => (
                    <div 
                      ...
                      className="flex items-center gap-2.5 p-2 bg-white rounded-xl border border-[#E4DFD3] hover:border-[#616F3E]/40 transition-colors cursor-pointer"
                    >
                      <div className="w-10 h-10 rounded-lg overflow-hidden shrink-0 border-2 border-white shadow-sm">
                        <img src={prod.image} alt={prod.name} className="w-full h-full object-cover" loading="lazy" decoding="async" />
                      </div>
                      <div className="min-w-0">
                        <h4 className="font-serif text-[11px] font-bold text-[#203348] truncate">{prod.name}</h4>
                        <p className="text-[9px] font-mono text-[#203348]/50">₹{prod.price}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="space-y-2">
                <h4 className="font-serif text-sm font-bold text-[#203348]">Why they work great together</h4>
                <p className="text-xs text-[#203348]/70 leading-relaxed bg-[#616F3E]/5 p-4 rounded-xl border border-[#616F3E]/15">
              ...
              <div className="space-y-3">
                <h4 className="font-serif text-sm font-bold text-[#203348]">When to take them</h4>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <div className="border border-[#E4DFD3] rounded-xl p-3 bg-white">
                    <span className="text-[8px] font-mono uppercase text-[#203348]/40 font-bold block mb-1">Morning</span>
                    <p className="text-[10px] text-[#203348]/70 leading-relaxed italic">{selectedDetailStack.schedule.morning}</p>
                  </div>
                  <div className="border border-[#E4DFD3] rounded-xl p-3 bg-white">
                    <span className="text-[8px] font-mono uppercase text-[#203348]/40 font-bold block mb-1">Afternoon</span>
                    <p className="text-[10px] text-[#203348]/70 leading-relaxed italic">{selectedDetailStack.schedule.afternoon}</p>
                  </div>
                  <div className="border border-[#E4DFD3] rounded-xl p-3 bg-white">
                    <span className="text-[8px] font-mono uppercase text-[#203348]/40 font-bold block mb-1">Evening</span>
                    <p className="text-[10px] text-[#203348]/70 leading-relaxed italic">{selectedDetailStack.schedule.evening}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            {/* Modal Footer */}
            <div className="p-6 bg-white border-t border-[#E4DFD3] flex flex-col sm:flex-row justify-between items-center gap-4 text-left">
              <div>
                <span className="text-[9px] font-mono text-[#203348]/40 uppercase block">Combo Price</span>
                <div className="flex items-baseline gap-2">
                  <span className="font-mono text-xl font-bold text-[#203348]">
                    ₹{selectedDetailStack.comboPrice}
                  </span>
                  <span className="font-mono text-xs text-[#203348]/40 line-through">₹{selectedDetailStack.originalPrice}</span>
                </div>
              </div>
              ...
              <button
                ...
                className="w-full sm:w-auto bg-[#A5492B] text-white hover:bg-[#203348] px-8 py-3.5 rounded-full text-xs font-semibold uppercase tracking-widest transition-all duration-300 shadow-md cursor-pointer text-center"
              >
                Buy Combo Stack
              </button>
            </div>
